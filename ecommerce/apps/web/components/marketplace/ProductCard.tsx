...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <div
      className="group relative bg-white rounded-2xl border border-slate-200/90 shadow-xs hover:shadow-xl hover:border-blue-300 transition-all duration-300 flex flex-col justify-between overflow-hidden"
    ...
    >
      <div>
        ...
        <div className="relative w-full aspect-square bg-slate-100 overflow-hidden">
          ...
          <div className="absolute top-2.5 left-2.5 flex flex-col gap-1.5 z-10 pointer-events-none">
            {product.discount_percentage > 0 && (
              <span className="px-2 py-0.5 rounded-md font-black text-[10px] bg-rose-600 text-white shadow-xs tracking-wider">
                {product.discount_percentage}% OFF
              </span>
            )}
            {product.badge && product.badge !== "FLASH SALE" && (
              <span
                className={`px-2 py-0.5 rounded-md font-bold text-[10px] text-white shadow-xs uppercase tracking-wider ${
        ...
        <div className="p-3.5 sm:p-4 flex flex-col gap-1.5">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1 text-[11px] font-semibold text-slate-500 hover:text-blue-600 transition-colors w-fit"
          >
          ...
          {/* Product Title */}
          <Link href={`/products/${product.slug}`} className="group-hover:text-blue-600 transition-colors">
          ...
          {product.is_free_delivery && (
            <div className="flex items-center gap-1 text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded w-fit mt-1">
      ...
      <div className="p-3.5 pt-0">
        <button
          ...
          className="w-full py-2 px-3 bg-slate-100 hover:bg-blue-600 text-slate-700 hover:text-white font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-colors shadow-xs"
        >
