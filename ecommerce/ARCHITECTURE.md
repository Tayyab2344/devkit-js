...
- **DB:** PostgreSQL / SQLite (for dev testing).
...
```
User          id, first_name, last_name, email, phone, password_hash, role(CUSTOMER|COMPANY|SUPER_ADMIN), is_active, is_verified, created_at, updated_at
...
- [x] Can coupons stack, or one per order? -> **Default: One coupon per order.**
- [x] Does a company go live immediately on signup, or does Super Admin approve first? -> **Default: Requires Super Admin approval (`status: pending` → `active`).**
- [x] Who funds a platform-wide coupon — platform absorbs it, or deducted proportionally from company payouts? -> **Default: Platform absorbs cost.**
- [x] Payment gateway — Stripe Connect fits this multi-vendor payout model well if you need split payouts. -> **Default: Stripe Connect.**

## 9. Integrations

| Integration | Purpose | Owner |
|---|---|---|
| Stripe Connect | Checkout + vendor payouts | DevOps |
